...
def process_image(
    ...
    for e in ellipse_info:
        notes_info.append({
            "id": int(e.get("sorted_id", 0)),  # Convert to Python int
            "staff_id": int(e.get("staff_id", 0)) if e.get("staff_id") is not None else 0,  # Convert to Python int
            "note_name": str(e.get("note_name", "Unknown")),  # Ensure string
            "type": str(e.get("type", "đen")),  # Ensure string
            "stem": str(e.get("stem", "không đuôi")),  # Ensure string
            "hook": str(e.get("hook")) if e.get("hook") is not None else None,  # Ensure string or None
